...
<link rel="stylesheet" href="${base}/resources/css/custom.css" type="text/css"></link>
 ...
 <div class="page-group">
  ...
  <header class="bar bar-nav apptitle">
     ...
     <h1 class="title title-color">校方认证</h1>
  ...
  <div class="content no-bottom white-bg-color">
    <!-- <form action="campusAuthentication" method="post"> -->
  	<div class="campus-authentication-info">
  		...
  		<div class="item-input">
  			<div class="item-input-content " ><input type="text" class="not-null" id="realName" name="realName" placeholder="请输入姓名"  value="" /></div>
  		</div>
  	</div>
  	<button class="primary-btn long-btn" id="campusAuthentication">立即认证</button>
  	<!-- </form> -->
  </div>
</div>
...
document.getElementById("campusAuthentication").addEventListener("click", function(event){
	  ...
	  var userType=$("#userType").val();
		...
		$.ajax({
			...
			type:"post",
			data:{"userType":userType,"userNumber":userNumber,"realName":realName},
			dataType:"json",
			beforeSend:function(){
			...
			success:function(data){
				clearTimeout(timer);
				 _$.hidePreloader();
				if (data.code=="0") {
					_$.router.load("toSettingPwd?id="+data.id+"&userType="+data.userType,true);
				}else{
